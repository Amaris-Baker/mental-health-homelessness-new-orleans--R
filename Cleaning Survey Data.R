...
# Note: Original CSV file not included in repository for privacy
Survey <- read.csv("HTC_Mental_Health_Survey_Data.csv")
names(Survey)
Survey <- Survey [-c(1:2),-c(1:18)]
Survey <- Survey [-c(88), ]
Survey [13,]
#The data points in row 2 and columns 1-18 were not needed for data analysis and were provided by Qualtrics
#Clean variables such as gender, race, ethnicity ect.

# Gender- the one 3 or other was someone identifying as Bisexual
unique(Survey$Q3)
Survey <- Survey %>%
...
# Types of Mental Healthcare Services Accessed (Q18) - multiselect
unique(Survey$Q18)
Survey <- Survey %>%
  mutate(
...
#Frequency of accessing mental health services
Survey <- Survey %>%
...
#Were the services interrupted within the past year?
Survey <- Survey%>%
...
# Causes of Service Interruption (Q31) - multiselect
unique(Survey$Q31)
Survey <- Survey %>%
  mutate(
...
# Barriers to Mental Healthcare (Q33) - multiselect
unique(Survey$Q33)
Survey <- Survey %>%
  mutate(
...
#Difficulty Accessing Mental Healthcare Services-Q35
Survey <- Survey %>%
...
#Q36 Do you feel that there are enough mental healthcare resources specifically tailored to meet the needs of unhoused individuals in New Orleans?
Survey <- Survey %>%
...
#Q37 Have you received any support through community organizations or shelters to address your mental health needs?
...
write.csv(Survey, "Survey_Cleaned_Final.csv", row.names = FALSE)

# Optional: Save as R data file (preserves data types)
saveRDS(Survey, "Survey_Cleaned_Final.rds")

# Create a summary of all cleaned variables
cat("=== CLEANED DATASET SUMMARY ===\n")
cat("Total observations:", nrow(Survey), "\n")
cat("Total variables:", ncol(Survey), "\n\n")

# List key cleaned variables
cleaned_vars <- c("gender", "ethnicity", "race_category", "time_homeless", "age_group", 
                  "sleep_setting", "tobacco_use", "alcohol_use", "other_drug_use", "any_substance_use",
                  "mh_category", "accessed_care", "services_interupted", "provider_category", 
                  "services_category", "mh_access_freq", "interruption_category", "barrier_category", 
                  "difficulty_accessing", "mh_resources_feelings", "support", "support_category")

cat("Key cleaned variables:\n")
...
cat("\nDataset saved as:\n")
cat("- Survey_Cleaned_Final.csv (for Excel/other software)\n")
cat("- Survey_Cleaned_Final.rds (for R - preserves data types)\n")
